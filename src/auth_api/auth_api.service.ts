...
export class AuthApiService {
  ...
  async login(@Body() user: CreateAuthApiDto): Promise<any> {
    try {
      ...
      console.log(
        'Header Token Workday:',
        header_token_workday.data.access_token,
      );
        ...
        if (user_data) {
          ...
          try {
            ...
            throw new UnauthorizedException(
              'Invalid credentials or module not found',
            );
          }
        } else {
      ...
      // Find Data
      return await this.userData.generateUserData(
        user_data,
        header_token,
      ...
      throw new UnauthorizedException(
        error.message /*'Invalid credentials or external auth failed'*/,
      );
    }
  }
}
